...
(() => {
  ...
  // Split on `;` only at the top level. A `;` inside url(...), a quoted string
  // or any other parenthesised value belongs to the value, not between
  // declarations. Splitting naively tore `url(data:image/webp;base64,...)` in
  // half, leaving an unterminated `url(data:image/webp` that painted nothing.
  ...
  function createExternalModules(onResolved) {
    ...
    function load(kind, url, after) {
      ...
      const p = ready.then(() => {
        // Cross-origin plain-js bundles: a classic <script> tag needs no CORS
        // headers, unlike fetch(). The bundle assigns its component to window;
        // capture the new globals exactly like the eval path below does.
        if (kind === "js" && /^https?:\/\//.test(url)) {
          ...
          return new Promise((res2, rej2) => {
            ...
            s.async = false;
            s.onload = () => res2();
            s.onerror = () => rej2(new Error("script tag load failed: " + url));
            document.head.appendChild(s);
          }).then(() => {
            const globals = {};
            ...
            console.info("[dc-runtime] x-import: loaded via script tag", url, "— globals:", Object.keys(globals));
            onResolved();
            return null;
          });
        }
